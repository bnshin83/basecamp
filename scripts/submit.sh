...
if [ -z "$SCRIPT" ]; then
    ...
    echo ""
    echo "Examples:"
    echo "  ./submit.sh train.py"
    echo "  ./submit.sh train.py rcac --gres=gpu:2 --time=48:00:00"
    echo "  ./submit.sh job.sbatch"
    exit 1
fi
...
EOF
    ...
    JOB_ID=$(ssh $CLUSTER "cd ~/projects/\$(basename \$PWD) && sbatch <<'HEREDOC'
$SBATCH_SCRIPT
HEREDOC" | grep -oE '[0-9]+')
    ...
    JOB_ID=$(ssh $CLUSTER "cd ~/projects/\$(basename \$PWD) && sbatch $SCRIPT" | grep -oE '[0-9]+')
fi
...
echo "  ssh $CLUSTER 'tail -f /scratch/\$USER/logs/${JOB_ID}_*.out'"
echo ""
echo "Cancel:"
echo "  ssh $CLUSTER 'scancel $JOB_ID'"

# Log
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) SUBMIT job=$JOB_ID script=$SCRIPT cluster=$CLUSTER" >> "$BASECAMP_DIR/logs/jobs.log"
